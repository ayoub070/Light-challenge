...
void loop() {
  ...
  if(BERICHT == HIGH){
    ...
    //checks if msg is for this pico
    if(rec.startsWith("1")) //change number for number of pico
    {
      Serial.println("Msg is for me");
      Serial2.println("Hoi");  
      ...
      if(rec.substring(17, 18) == "0"){ // 17 -> 0 is for default keepalive check 
        Serial2.println("1alive");
        Serial.println("1alive");
      }
        ...
        Serial2.println("1done");
      }
      else if(rec.substring(17,18 ) == "2"){ //17 -> 2 is for sending data to the master
        Serial.println("1Blablabla dit is de data");
        Serial2.println("1Blablabla dit is de data");
      }
